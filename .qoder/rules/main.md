...
3. **先查后建**：收到缺陷或需求时，先 `gh issue list --search` 搜索相关 Issue；若已存在则查看进度（是否已分配、已有 PR、已关闭），避免重复工作。只有确认无重复后才新建 Issue。
4. **分支与 PR**：**禁止直接推 `main`**；按 Issue 切短期分支，PR 用 `Fixes #编号` 关联。
5. **文档归位**：所有文档使用相关工具并按主题分类，根目录仅保留 `README.md`。
6. **远端协作**：终端与 GitHub 交互优先使用 **GitHub CLI（`gh`）**；不在仓库写入 token。
7. **同步更新**：行为或接口变化时，**代码与文档同提交**，避免文档过期。
8. **模块化开发**：优先保证高内聚、低耦合；复杂逻辑必须拆分为独立模块或函数方法，避免巨型文件与巨型函数。
...
- **Commit message 必须使用中文描述**，格式为 `<type>(scope): <中文说明>`，例如：
  - `fix(models): 修改租户管理员和用户的删除策略`
  - `feat(ai-panel): 路由切换时实时刷新页面上下文`
  - `refactor(config): 优化版本号读取逻辑`
  - `docs(rules): 新增任务前置调查与查重规则`
  其中 `type` 取值：`feat` / `fix` / `refactor` / `docs` / `chore` / `perf` / `test`；`scope` 为影响范围（模块/目录）；描述部分用简洁中文说明变更内容。
