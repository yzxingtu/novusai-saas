...
- [backend/core/config.py](file://backend/app/core/config.py)
...
## 更新摘要
**变更内容**
- 更新镜像注册表为GitHub Container Registry (GHCR)
- 新增GitHub Actions CI/CD工作流配置
- 更新镜像命名空间为ghcr.io/yzxingtu/novusai-*
- 新增镜像构建和推送自动化流程
...
**更新** 本版本反映了生产环境镜像注册表从自定义注册表迁移到GitHub Container Registry (GHCR)，所有后端和前端镜像都已更新为ghcr.io/yzxingtu/novusai-*命名空间，并集成了GitHub Actions自动化构建和推送流程。
...
- **镜像管理**：基于GitHub Container Registry (GHCR)的容器镜像管理，支持多架构构建和自动化推送。
...
- **镜像注册表**：所有镜像现在使用GitHub Container Registry (GHCR)作为统一注册表，镜像命名空间为ghcr.io/yzxingtu/novusai-*。

**更新** 镜像注册表已从自定义注册表迁移到GitHub Container Registry (GHCR)，所有镜像都使用ghcr.io/yzxingtu/novusai-*命名空间。
...
- **自动化构建**：当推送到带有版本标签的分支时，自动触发镜像构建和推送流程。
- **多架构支持**：同时构建linux/amd64和linux/arm64架构镜像，确保跨平台兼容性。
- **镜像命名**：使用ghcr.io/${{ github.repository_owner }}/${{ matrix.name }}的命名规范。
- **安全认证**：使用GitHub Token进行GHCR登录，无需额外的凭据管理。
- **标签管理**：自动为每个Git标签创建对应镜像标签，支持latest标签。

**新增** GitHub Actions工作流提供了完整的CI/CD自动化，包括镜像构建、多架构支持和安全推送。
...
## 结论
通过上述容器化与编排策略、安全与监控体系、备份与恢复方案，以及性能调优与故障恢复最佳实践，NovusAI SaaS可在生产环境中实现高可用、可扩展与可维护的部署目标。**更新** 通过GitHub Container Registry (GHCR)的镜像管理、GitHub Actions的自动化构建和推送，以及多架构支持，进一步提升了部署的可靠性和效率。建议在上线前完成完整的端到端演练与安全审计，并建立持续改进的运维流程。
...
- **镜像管理**：使用GHCR作为统一镜像注册表，支持多架构构建、自动化推送和版本管理。
- **安全加固**：GitHub Actions使用GitHub Token进行安全认证，镜像构建过程透明可追溯。