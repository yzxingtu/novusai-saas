...
38. `backend/app/ai/memory_policy.py`、`backend/app/ai/context/orchestrator.py`、`backend/app/services/ai/agent_chat_memory_support.py`、`backend/app/services/ai/conversation_message_persistence_service.py`、`backend/app/services/ai/conversation_message_persistence_support.py` 与 `backend/app/services/ai/conversation_runtime_projection_service.py` 已把 `WS5 thread-memory-owner` 的主链再推进一刀：统一 `memory_runtime_policy` / `thread_memory_state` 现在会携带 machine-readable `thread_memory_owner_state`、`thread_memory_owner_reason`、`session_memory_state`、`long_term_memory_recall_state`、`long_term_memory_capture_state`，startup priming、polluted-turn recall/capture 降级、assistant metadata 持久化、thread snapshot 写回，以及 conversation detail / error-only fallback 投影都改为消费这条 canonical owner state，而不是继续各自从布尔 flag 或 call-site 私有逻辑重推状态。
...
### 2026-04-21 WS4 closeout update
...
2. `backend/app/sio/page_session.py` 现已删除 `(scope, user_id, page_key)` 活跃会话索引与 socket 级 cleanup 追踪；`ui_action_executor`、`page_runtime` bridge 与 `invoke_ui_action(...)` 也都会在进入 live transport 之前剔除 `page_key`，因此旧的 page-key 身份链不再进入 action payload。
3. `frontend/apps/web-antd/src/components/business/ai-chat-panel/use-ai-chat-page-operations.ts` 已删除 `suggested_tools -> hasInteractivePageContext` 这条 live gating seam；page-operation channel readiness 现在只认真实 runtime state（例如 `ui_epoch`、active surface、active form）。
