...
| 信任会话持久化 | 通过 | loadConversationMessages 成功后从 sessionStorage 读取 `ai_trust_session_${convId}` 写回 trustSession；watch([trustSession, activeConversationId]) 在变化时写回，且 `convId != null && typeof convId === 'number'` 才写入 |
...
### 2.4 新建会话时 trustSession 不重置

- **现状**：startNewConversation 将 activeConversationId 置 null、清空 chatMessages，但不重置 trustSession。
- **影响**：从会话 A（勾选信任）切到「新对话」时，复选框仍为勾选；发送首条后新会话会以该状态写入 sessionStorage。行为可接受，仅视觉上「新对话」仍沿用上一会话的勾选状态。
- **可选**：若产品希望「新对话」默认不信任，可在 startNewConversation 中增加 `trustSession.value = false`。
...
- **useAIChat**：仅上述两处与 command-bar 相关逻辑使用；command-bar 仅负责打开面板与提交文案，不直接依赖 retryLastMessage/trustSession 等新增行为，无回归风险。
...
- **建议修复/增强**（按优先级）：
  ...
  4. **可选**：startNewConversation 时重置 trustSession，使「新对话」默认不信任。
