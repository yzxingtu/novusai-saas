...
def upgrade():
    ...
    if 'px_storage_migration_tasks' not in existing_tables:
        op.create_table(
    ...
    if 'px_storage_migration_logs' not in existing_tables:
        op.create_table(
    ...
    existing_indexes = {idx.get("name") for idx in inspector.get_indexes("px_storage_migration_logs")} if 'px_storage_migration_logs' in existing_tables else set()
    if 'ix_px_sm_logs_task_status' not in existing_indexes:
        op.create_index(
    ...
    existing_indexes = {idx.get("name") for idx in inspector.get_indexes("px_storage_migration_tasks")} if 'px_storage_migration_tasks' in existing_tables else set()
    if 'ix_px_sm_tasks_status' not in existing_indexes:
        op.create_index(
            'ix_px_sm_tasks_status',
            'px_storage_migration_tasks',
            ['status'],
        )
    if 'ix_px_sm_tasks_created_at' not in existing_indexes:
        op.create_index(
