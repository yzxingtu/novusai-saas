    ...
    if "px_storage_migration_tasks" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                ...
                sa.String(50),
                nullable=False,
                comment="Source storage driver",
            ),
            sa.Column(
                "target_driver",
                sa.String(50),
                nullable=False,
                comment="Target storage driver",
            ),
            sa.Column(
                "status",
                sa.String(20),
            ...
            sa.Column(
                "created_by", sa.Integer(), nullable=False, comment="Admin user ID"
            ),
            sa.Column(
...
def downgrade():
    ...
    if "ix_px_sm_tasks_created_at" in task_indexes:
        op.drop_index(
            "ix_px_sm_tasks_created_at", table_name="px_storage_migration_tasks"
        )
    ...
    if "ix_px_sm_logs_task_status" in log_indexes:
        op.drop_index(
            "ix_px_sm_logs_task_status", table_name="px_storage_migration_logs"
        )
    if "px_storage_migration_logs" in existing_tables:
