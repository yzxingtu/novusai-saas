...
from app.ai.engine.tool_followup_runner import ToolFollowupDecision
from app.ai.engine.tool_loop_session import ToolLoopSession
from app.ai.engine.tool_round_execution_helpers import ToolRoundExecutionOutcome
...
@pytest.mark.asyncio
async def test_run_tool_call_loop_truncates_navigation_batch_before_round_execution(
    monkeypatch,
) -> None:
    navigate_call = {
        "id": "tc-nav",
    ...
    response = _make_response([navigate_call, stale_call])
    tools = [
        ToolDefinition(name="ui_open_surface", description="navigate"),
    ...
    final_response = ChatResponse(
        message=ChatMessage(role="assistant", content="navigated"),
        total_tokens=9,
        output_tokens=4,
    )
    ...
    async def _fake_execute_tool_round(**kwargs):
        ...
        return ToolRoundExecutionOutcome(tracked_tool_result_bytes=0)

    async def _fake_run_tool_followup(**_kwargs):
    ...
    monkeypatch.setattr(runtime_mod, "execute_tool_round", _fake_execute_tool_round)
    monkeypatch.setattr(
        runtime_mod,
        "apply_round_recovery_and_focus",
        lambda **_kwargs: None,
    )
    monkeypatch.setattr(runtime_mod, "run_tool_followup", _fake_run_tool_followup)

    result = await runtime_mod.run_tool_call_loop(
        runtime=runtime_mod.ToolCallLoopRuntime(
            sandbox=MagicMock(),
            agent=SimpleNamespace(id=1),
            messages=[ChatMessage(role="user", content="open the page")],
            response=response,
            tools=tools,
            all_tools=tools,
            request=_make_request(),
        ...
        callbacks=_make_callbacks(
            call_followup_llm=AsyncMock(return_value=final_response),
            truncate_tool_calls_after_navigation=lambda tool_calls: (
                [tool_calls[0]],
                True,
            ),
        ),
    ...
    assert seen["round_tool_calls"] == [navigate_call]
    assert seen["assistant_tool_calls"] == [navigate_call]
    assert response.tool_calls == [navigate_call]
    assert result == (final_response, [], 9, 4)
