...
Scope: OpenAI-compatible client factory retry ownership.
...
def test_resolve_retry_client_preserves_zero_sdk_retries(monkeypatch) -> None:
    captured_kwargs: list[dict[str, object]] = []
    ...
    client, retry_base_url = resolve_retry_client(
        api_key="test-key",
    ...
    assert retry_base_url == "https://api.example.com/v1"
    assert captured_kwargs == [
        {
            "api_key": "test-key",
            "base_url": "https://api.example.com/v1",
