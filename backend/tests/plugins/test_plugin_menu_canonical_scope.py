...
def test_register_menu_stores_canonical_scopes() -> None:
    ...
    try:
        ...
        assert menus == {"adm": "admin", "ten": "tenant"}
    finally:
...
def test_register_menu_rejects_user_scope_for_plugins() -> None:
    reg = ExtensionRegistry.get_instance()
    pname = "menu_scope_user"
    with pytest.raises(ValueError, match="Invalid plugin menu scope"):
        reg.register_menu(pname, "legacy-user", "/x", scope="user")
