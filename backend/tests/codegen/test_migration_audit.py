...
def _setup_manifest(
    tmp_path: Path, resource: str, files: list[dict], migration_file: str | None = None
) -> None:
    """写入 manifest 条目."""
...
def test_partial_rollback_sets_success_false_and_preserves_manifest(
    tmp_path: Path,
) -> None:
        ...
        [
            {
                "path": "routers.py",
                "action": "append",
                "appended_content": "router.include_router(x)",
            }
        ],
    )
...
def test_run_rollback_migration_cleanup_refuses_non_head_revision(
    tmp_path: Path,
) -> None:
...
def test_register_model_failure_surfaces_error_and_blocks_auto_migrate(
    tmp_path: Path,
) -> None:
