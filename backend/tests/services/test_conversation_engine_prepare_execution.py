...
@pytest.mark.asyncio
async def test_prepare_execution_injects_ephemeral_rag_context_and_sources() -> None:
    engine = ConversationEngine(db=MagicMock(), gateway=MagicMock(), sandbox=MagicMock())
    agent = _build_agent()
    request = ExecutionRequest(
        agent_id=1,
        ...
        user_id=7,
        messages=[
            ChatMessage(role="user", content="请总结这份接口文档里的认证方式"),
        ],
        input_variables={},
        ephemeral_rag_refs=[
            {
                "kind": "text",
                "title": "接口说明",
                "content": "认证方式: OAuth 2.0 + JWT token。所有请求需带 Authorization 头。",
            }
        ],
    )
    ...
    assert "[EPHEMERAL DOCUMENT CONTEXT]" in prep.messages[0].content
    assert "接口说明" in prep.messages[0].content
    assert "ephemeral_doc" in prep.rag_source_kinds
    assert prep.rag_sources


@pytest.mark.asyncio
async def test_prepare_execution_builds_ephemeral_rag_from_text_attachment() -> None:
    engine = ConversationEngine(db=MagicMock(), gateway=MagicMock(), sandbox=MagicMock())
    agent = _build_agent()
    request = ExecutionRequest(
        agent_id=1,
                ...
                attachments=[
                    {
                        ...
                        "type": "file",
                        "url": "/api/public/attachments/321/access",
                    }
                ],
            ),
        ],
    ...
    with (
        ...
        patch("app.ai.routing.router.ModelRouter", new=_FakeRouter),
        patch(
            "app.ai.context.ephemeral_rag.EphemeralRAGProvider._attachment_to_ref",
            new=AsyncMock(
                return_value={
                    "kind": "text",
                    "doc_id": 321,
                    "title": "auth-note.txt",
                    "content": "认证方式: OAuth 2.0 + JWT。",
                    "source_ref": "attachment:321",
                }
            ),
        ),
    ):
    ...
    assert "[EPHEMERAL DOCUMENT CONTEXT]" in prep.messages[0].content
    assert "auth-note.txt" in prep.messages[0].content
    assert "ephemeral_doc" in prep.rag_source_kinds
