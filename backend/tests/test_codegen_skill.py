...
class TestSkillDefinitions:
    def test_tool_count(self):
        """11 个 Tool 定义"""
        assert len(CRUD_TOOL_DEFINITIONS) == 11
    ...
    def test_tool_names(self):
        ...
        expected = {
            ...
            "crud_analyze_intent",
            "crud_batch_generate_config",
            "crud_batch_preview",
            "crud_batch_generate_files",
        }
...
# ============================================================
# BatchCrudProject Schema 测试
# ============================================================


class TestBatchCrudProjectSchema:
    """BatchCrudProject / EntityRelation Schema 测试"""
    ...
    def test_batch_project_minimal(self):
        """最小化 BatchCrudProject 实例化"""
        from app.codegen.schemas import BatchCrudProject

        project = BatchCrudProject(
            project_name="订单管理",
            entities=[self._ENTITY_A],
        )
        assert project.project_name == "订单管理"
        assert len(project.entities) == 1
        assert project.entities[0].module == "order"
        assert project.cross_relations == []
        assert project.shared_enums == []
        assert project.generation_order == []

    def test_batch_project_multi_entity(self):
        """多实体 BatchCrudProject"""
        from app.codegen.schemas import BatchCrudProject

        project = BatchCrudProject(
            project_name="订单管理",
            entities=[self._ENTITY_A, self._ENTITY_B],
            generation_order=["order", "order-item"],
        )
        assert len(project.entities) == 2
        assert project.generation_order == ["order", "order-item"]

    def test_batch_project_with_cross_relations(self):
        """跨表关联"""
        from app.codegen.schemas import BatchCrudProject, EntityRelation
        ...
        assert len(project.cross_relations) == 1
        rel = project.cross_relations[0]
        assert rel.source_entity == "order-item"
        assert rel.target_entity == "order"
        assert rel.relation_type.value == "belongs_to"
        assert rel.foreign_key == "order_id"
        assert rel.nullable is False

    def test_batch_project_with_shared_enums(self):
        """共享枚举"""
        from app.codegen.schemas import BatchCrudProject
            ...
            shared_enums=[
                {
                    "name": "PaymentMethod",
                    "values": [
                        {"value": "cash", "label_zh": "现金", "label_en": "Cash"},
                        {"value": "card", "label_zh": "银行卡", "label_en": "Card"},
                    ],
                }
            ],
        )
        assert len(project.shared_enums) == 1
        assert project.shared_enums[0].name == "PaymentMethod"

    def test_batch_project_empty_entities_fails(self):
        """空 entities 列表校验失败"""
        from app.codegen.schemas import BatchCrudProject
    ...
    def test_entity_relation_defaults(self):
        """EntityRelation 默认值"""
        from app.codegen.schemas import EntityRelation
    ...
    def test_batch_project_serialization(self):
        """BatchCrudProject JSON 序列化/反序列化"""
        from app.codegen.schemas import BatchCrudProject, EntityRelation
        ...
        assert data["project_name"] == "测试"
        assert len(data["entities"]) == 2
        assert len(data["cross_relations"]) == 1

        # 反序列化
        restored = BatchCrudProject(**data)
...
class TestGenerateBatch:
    """CrudGenerator.generate_batch() 多表批量生成测试"""
    ...
    def test_batch_generates_multiple_entities(self):
        """批量生成多个实体的文件"""
        ...
        project = BatchCrudProject(
            project_name="订单管理",
            entities=[self._ENTITY_ORDER, self._ENTITY_ORDER_ITEM],
        )
        gen = CrudGenerator()
        files = gen.generate_batch(project)

        # 应有两个实体的文件
        order_files = [p for p in files if "order" in p and "order_item" not in p and not p.startswith("__")]
        item_files = [p for p in files if "order_item" in p and not p.startswith("__")]
        assert len(order_files) > 0
        assert len(item_files) > 0

    def test_batch_has_entity_file_map(self):
        """批量生成包含 __entity_file_map__.json 元数据"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import BatchCrudProject

        project = BatchCrudProject(
            project_name="测试",
            entities=[self._ENTITY_ORDER, self._ENTITY_PRODUCT],
        )
        gen = CrudGenerator()
        files = gen.generate_batch(project)

        assert "__entity_file_map__.json" in files
        file_map = json.loads(files["__entity_file_map__.json"])
        assert isinstance(file_map, dict)
        assert len(file_map) > 0

        # 验证归属标记
        modules_in_map = set(file_map.values())
        assert "order" in modules_in_map
        assert "product" in modules_in_map

    def test_batch_combined_ddl(self):
        """联合 DDL 预览包含所有表"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import BatchCrudProject
        ...
        assert "联合 DDL 预览" in ddl
        assert "orders" in ddl
        assert "products" in ddl

    def test_batch_cross_relation_injection(self):
        """跨表关联注入到实体 relations"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import BatchCrudProject, EntityRelation
        ...
        # 验证 order_item 的 model 文件包含外键
        item_model_path = [p for p in files if "order_item" in p and "model" in p]
        assert len(item_model_path) > 0

    def test_batch_generation_order(self):
        """generation_order 控制 DDL 排列顺序"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import BatchCrudProject

        project = BatchCrudProject(
            project_name="测试",
            entities=[self._ENTITY_ORDER_ITEM, self._ENTITY_ORDER],
            generation_order=["order", "order-item"],
        )
        gen = CrudGenerator()
        files = gen.generate_batch(project)

        ddl = files["__ddl_preview__.sql"]
        # order 应在 order_items 之前
        order_pos = ddl.find("orders")
        item_pos = ddl.find("order_items")
        assert order_pos < item_pos

    def test_batch_3_entities(self):
        """3 个实体批量生成"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import BatchCrudProject, EntityRelation
        ...
        project = BatchCrudProject(
            ...
            entities=[
                self._ENTITY_ORDER,
                self._ENTITY_ORDER_ITEM,
                self._ENTITY_PRODUCT,
            ],
            cross_relations=[
                EntityRelation(
                    source_entity="order-item",
            ...
            generation_order=["product", "order", "order-item"],
        )
        gen = CrudGenerator()
        files = gen.generate_batch(project)
        ...
        # 应有 3 个实体的文件
        file_map = json.loads(files["__entity_file_map__.json"])
        modules = set(file_map.values())
        assert "order" in modules
        assert "order-item" in modules
        assert "product" in modules

        # DDL 中 3 个表
        ddl = files["__ddl_preview__.sql"]
        assert "orders" in ddl
        assert "order_items" in ddl
        assert "products" in ddl

    def test_batch_preview_grouped_by_entity(self):
        """preview_batch 按实体分组"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import BatchCrudProject
        ...
        writer = CrudWriter("/tmp/nonexistent")
        result = writer.preview_batch(files)
        ...
        for entity in result["entities"]:
            assert entity["file_count"] > 0
            assert len(entity["files"]) == entity["file_count"]

        # total 统计
        assert result["total_files"] > 0
        assert isinstance(result["total_new"], int)
        assert isinstance(result["total_conflict"], int)
        assert isinstance(result["ddl_preview"], str)

    def test_batch_preview_fallback_without_map(self):
        """preview_batch 无元数据时退化为单组"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import CrudConfig
        from app.codegen.writer import CrudWriter

        # 单表 generate() 不含 __entity_file_map__.json
        config = CrudConfig(**self._ENTITY_ORDER)
        gen = CrudGenerator()
        ...
        assert result["entities"][0]["entity_name"] == "_single"
        assert result["entities"][0]["file_count"] > 0

    def test_batch_preview_has_ddl(self):
        """preview_batch 包含联合 DDL"""
        from app.codegen.generator import CrudGenerator
        from app.codegen.schemas import BatchCrudProject
        from app.codegen.writer import CrudWriter
        ...
        assert "联合 DDL 预览" in result["ddl_preview"]
        assert "orders" in result["ddl_preview"]
        assert "order_items" in result["ddl_preview"]
...
class TestExecutorBatchMethods:
    """CrudGeneratorExecutor 批量 Tool 测试"""
    ...
    @pytest.mark.asyncio
    async def test_batch_preview_executor(self):
        """crud_batch_preview 通过 Executor 执行"""
        from app.ai.tools.executors.crud_generator_executor import (
            CrudGeneratorExecutor,
        )

        executor = CrudGeneratorExecutor()
        defn = self._make_definition("crud_batch_preview")
        result = await executor.execute(
            defn,
            tool_call_id="tc_batch_1",
            arguments={"project": self._BATCH_PROJECT},
        )
        assert result.success is True
        data = json.loads(result.output)
    ...
    @pytest.mark.asyncio
    async def test_batch_generate_files_requires_confirmation(self):
        """crud_batch_generate_files 未确认时返回 requires_confirmation"""
        from app.ai.tools.executors.crud_generator_executor import (
            CrudGeneratorExecutor,
        )

        executor = CrudGeneratorExecutor()
        defn = self._make_definition("crud_batch_generate_files")
        result = await executor.execute(
            defn,
            tool_call_id="tc_batch_2",
            arguments={"project": self._BATCH_PROJECT, "confirmed": False},
        )
        assert result.success is True
        data = json.loads(result.output)
        assert data["requires_confirmation"] is True
    ...
    @pytest.mark.asyncio
    async def test_batch_generate_config_no_gateway(self):
        """crud_batch_generate_config 无 gateway 时优雅降级"""
        from app.ai.tools.executors.crud_generator_executor import (
            CrudGeneratorExecutor,
        )

        executor = CrudGeneratorExecutor(gateway=None)
        defn = self._make_definition("crud_batch_generate_config")
        result = await executor.execute(
            defn,
            tool_call_id="tc_batch_3",
            arguments={"description": "电商后台需要订单和商品模块"},
        )
        assert result.success is True
        data = json.loads(result.output)
        assert "error" in data
        assert "AI Gateway" in data["error"]

    @pytest.mark.asyncio
    async def test_batch_validate_all_tools(self):
        """validate 覆盖 3 个批量 Tool"""
        from app.ai.tools.executors.crud_generator_executor import (
            CrudGeneratorExecutor,
        )
        ...
        # batch_preview 需要 project
        defn = self._make_definition("crud_batch_preview")
        assert await executor.validate(defn, {"project": {}}) is True
        assert await executor.validate(defn, {}) is False

        # batch_generate_files 需要 project
        defn = self._make_definition("crud_batch_generate_files")
        assert await executor.validate(defn, {"project": {}}) is True

        # batch_generate_config 需要 description
        defn = self._make_definition("crud_batch_generate_config")
        assert await executor.validate(defn, {"description": "test"}) is True
        assert await executor.validate(defn, {}) is False
