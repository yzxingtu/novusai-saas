...
class AdminRoleService(GlobalService[AdminRole, AdminRoleRepository], RoleTreeMixin[AdminRole]):
    ...
    async def delete_role(self, role_id: int) -> bool:
        ...
        Raises:
            ...
            DependencyBlockedException: 有子角色或关联用户 / Has children or admins
        """
        role = await self.repo.get_by_id(role_id)
        if not role:
            raise NotFoundException(message=_("role.not_found"))
    ...
    async def create_member(
        ...
        # 使用 AdminService 创建管理员，直接关联到指定角色
        ...
        admin = await admin_service.create_admin(
            ...
            nickname=nickname,
            is_active=is_active,
            is_super=False,  # 通过组织架构创建的成员不能是超管
            role_id=role_id,
        )
