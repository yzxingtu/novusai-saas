...
class AdminDashboardService:
    ...
    async def get_recent_activities(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        refs = {
            ref
            for log in items
            if (ref := _operation_log_identity_ref(log)) is not None
        }
        identity_meta_map = await _load_operation_log_identity_meta_map(self.db, refs)

        return [
            _serialize_recent_activity(
                log,
                identity_meta_map.get(_operation_log_identity_ref(log)),
...
class TenantDashboardService:
    ...
    async def get_recent_activities(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        identity_meta_map = await _load_operation_log_identity_meta_map(
            self.db,
            refs,
