...
class ConversationDiagnosticsProjector:
    ...
    def copy_metadata(raw: Any) -> dict[str, Any] | None:
        return ConversationDiagnosticsProjector.normalize_json_safe_dict(raw)
    ...
    def normalize_turn_record_payload(turn_record: Any) -> dict[str, Any] | None:
        return ConversationDiagnosticsProjector.normalize_json_safe_dict(turn_record)
    ...
    def normalize_context_sources(value: Any) -> list[dict[str, Any]]:
        ...
        for item in value:
            if not isinstance(item, dict):
                continue
            source_name = ConversationDiagnosticsProjector.to_non_empty_str(
                item.get("name")
            )
            source_kind = ConversationDiagnosticsProjector.to_non_empty_str(
                item.get("kind")
            )
    ...
    @classmethod
    def normalize_intent_plan(cls, value: Any) -> list[dict[str, Any]]:
        if not isinstance(value, list):
            return []
        normalized: list[dict[str, Any]] = []
        for item in value:
            payload = cls.normalize_json_dict(item)
            if not payload:
                continue
            normalized.append(
                {
                    "intent_id": cls.to_non_empty_str(payload.get("intent_id")),
                    "kind": cls.to_non_empty_str(payload.get("kind")),
                    "family": cls.to_non_empty_str(payload.get("family")),
                    "order": int(payload.get("order") or 0) or None,
                    "user_visible_label": cls.to_non_empty_str(
                        payload.get("user_visible_label")
                    ),
                    "status": cls.to_non_empty_str(payload.get("status")),
                    "allowed_tool_names": cls.normalize_string_list(
                        payload.get("allowed_tool_names")
                    ),
                    "completed_by_tool_names": cls.normalize_string_list(
                        payload.get("completed_by_tool_names")
                    ),
                    "failure_reason": cls.to_non_empty_str(
    ...
    @classmethod
    def normalize_retry_events(cls, value: Any) -> list[dict[str, Any]]:
        if not isinstance(value, list):
            return []
        normalized: list[dict[str, Any]] = []
        for item in value:
            ...
            normalized.append(
                {
                    "action": cls.to_non_empty_str(payload.get("action")),
                    "target_intent_id": cls.to_non_empty_str(
                        payload.get("target_intent_id")
                    ),
                    "retry_family": cls.to_non_empty_str(payload.get("retry_family")),
                    "allowed_tool_names": cls.normalize_string_list(
                        payload.get("allowed_tool_names")
                    ),
                    "completed_intent_ids": cls.normalize_string_list(
                        payload.get("completed_intent_ids")
                    ),
                    "unfinished_intent_ids": cls.normalize_string_list(
                        payload.get("unfinished_intent_ids")
                    ),
                    "reason": cls.to_non_empty_str(payload.get("reason")),
                    "provider_failure_kind": cls.to_non_empty_str(
    ...
    @classmethod
    def normalize_provider_events(cls, value: Any) -> list[dict[str, Any]]:
        if not isinstance(value, list):
            return []
        normalized: list[dict[str, Any]] = []
        for item in value:
    ...
    def extract_turn_diagnostics_from_metadata(
        ...
        turn_record = cls.normalize_turn_record_payload(metadata.get("turn_record"))
        ...
        turn_outcome = cls.to_non_empty_str(
        ...
        termination_reason = cls.to_non_empty_str(
        ...
        metadata_completion_reason = cls.to_non_empty_str(
        ...
        if metadata_marks_partial:
            ...
            if (
                ...
                turn_outcome = "failed"
        protocol_path = cls.to_non_empty_str(
        ...
        selected_tool_names = cls.normalize_string_list(
        ...
        selected_skill_names = cls.normalize_string_list(
        ...
        context_sources = cls.normalize_context_sources(
        ...
        contract_breach_type = cls.to_non_empty_str(
        ...
        tool_planner = cls.normalize_json_dict(
        ...
        unfinished_intents = cls.normalize_string_list(
        ...
        leaked_tool_names = cls.normalize_string_list(
        ...
        last_tool_name = cls.to_non_empty_str(
        ...
        last_page_key = cls.to_non_empty_str(
        ...
        last_page_op = cls.to_non_empty_str(
        ...
        interrupted_stage = cls.to_non_empty_str(
        ...
        execution_path = cls.to_non_empty_str(
        ...
        active_intent_id = cls.to_non_empty_str(
        ...
        continuation_source = cls.to_non_empty_str(
        ...
        conversation_outcome = cls.to_non_empty_str(
        ...
        intent_plan = cls.normalize_intent_plan(
        ...
        budget = cls.normalize_json_dict(
        ...
        candidate_tool_names = cls.normalize_string_list(
        ...
        retry_events = cls.normalize_retry_events(
        ...
        partial_exit_reason = cls.to_non_empty_str(
        ...
        failure_kind = cls.to_non_empty_str(
            metadata.get("failure_kind")
            or turn_record_diagnostics.get("failure_kind")
            or (cls.normalize_json_dict(metadata.get("failures")) or {}).get(
                "failure_kind"
            )
            or (
                cls.normalize_json_dict(turn_record_diagnostics.get("failures")) or {}
            ).get("failure_kind")
            or context_diagnostics.get("failure_kind")
            or last_run_summary.get("failure_kind")
        )
        provider_events = cls.normalize_provider_events(
        ...
        budget_status = cls.to_non_empty_str(
        ...
        budget_exit_reason = cls.to_non_empty_str(
