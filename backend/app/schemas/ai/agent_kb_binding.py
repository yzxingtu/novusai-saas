...
class AgentKBBindRequest(BaseCreateSchema):
    ...
    weight: float = Field(
        1.0, ge=0.1, le=2.0, description=_("agent_kb_binding.field.weight")
    )
...
class AgentKBBindingUpdate(BaseUpdateSchema):
    ...
    weight: float | None = Field(
        None, ge=0.1, le=2.0, description=_("agent_kb_binding.field.weight")
    )
    enabled: bool | None = Field(None, description=_("agent_kb_binding.field.enabled"))
    sort_order: int | None = Field(
        None, ge=0, description=_("agent_kb_binding.field.sort_order")
    )
