...
class PlatformPublicConfig(BaseSchema):
    ...
    domain_verification_prefix: str | None = Field(
        None, description="域名验证 DNS 前缀"
    )
    ...
    captcha_enable_threshold_admin: int | None = Field(
        None, description="验证码启用阈值"
    )
    ...
    platform_domains: list[str] = Field(
        default_factory=list, description="平台管理端域名列表"
    )
