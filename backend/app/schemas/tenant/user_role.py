"""
企业用户角色相关 Schema / Tenant User Role Schema

定义企业用户角色管理的请求和响应数据结构（扁平结构，无层级）
...
class TenantUserRoleResponse(BaseSchema):
    """企业用户角色响应 / Tenant user role response."""
...
class TenantUserRoleDetailResponse(TenantUserRoleResponse):
    """企业用户角色详情响应（含权限） / Tenant user role detail response (with permissions)."""
...
class TenantUserRoleCreateRequest(BaseSchema):
    """创建企业用户角色请求 / Create tenant user role request."""
...
class TenantUserRoleUpdateRequest(BaseSchema):
    """更新企业用户角色请求 / Update tenant user role request."""
