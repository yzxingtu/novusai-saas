"""
企业管理端 WebSocket 相关 HTTP API / Tenant WebSocket HTTP API
...
async def get_tenant_presence(
    ...
    """
    获取当前企业的管理员在线状态 / Get admin online presence for current tenant
...
async def get_tenant_user_presence(
    ...
    """
    获取当前企业的业务用户在线状态 / Get business user online presence for current tenant

    自动从当前登录的企业管理员获取 tenant_id，
    Automatically gets tenant_id from the currently logged-in tenant admin,
    确保企业隔离（只能查看本企业的用户在线状态）。
