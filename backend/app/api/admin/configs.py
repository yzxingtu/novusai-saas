...
class AdminConfigController(GlobalController):
    ...
    def _register_routes(self) -> None:
        ...
        async def get_group_configs(
            ...
            configs = [_translate_config_item(c) for c in raw_configs]
        ...
        async def update_group_configs(
            ...
            configs = (
                [_translate_config_item(c) for c in raw_configs] if target_group else []
            )
        ...
        @router.get(
            "/storage/drivers/{driver_name}/schema", summary="获取存储驱动配置 Schema"
        )
        @action_read("action.platform_config.read")
