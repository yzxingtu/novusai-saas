...
async def get_global_preferences(
    ...
    prefs = await service.get_global_preferences("platform_global", tenant_id=0)
...
async def update_global_preferences(
    ...
    """
    更新平台全局通知偏好，变更的分类会从所有管理员个人覆盖中清除
    Update platform global notification preferences; changed categories are cleared from all admin overrides
    """
    service = NotificationPreferenceService(db)
...
async def get_preferences(
    ...
    service = NotificationPreferenceService(db)
    prefs = await service.get_all_preferences("admin", admin.id, tenant_id=0)
    return success(data=prefs)
...
async def save_preferences(
    ...
    service = NotificationPreferenceService(db)
    await service.save_preferences("admin", admin.id, data, tenant_id=0)
    await db.commit()
    return success(message=_("common.success"))
...
async def reset_preferences(
    ...
    service = NotificationPreferenceService(db)
    await service.reset_individual_preferences("admin", admin.id, tenant_id=0)
    await db.commit()
    return success(message=_("common.success"))
