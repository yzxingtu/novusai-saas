...
class Permission(BaseModel):
    ...
    type: Mapped[str] = mapped_column(
        String(20),
        index=True,
        comment="权限类型: menu/operation / Permission kind",
    )
    ...
    scope: Mapped[str] = mapped_column(
        String(20),
        index=True,
        comment="作用域: admin/tenant/both / Scope",
    )
    ...
    icon: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="图标（菜单专用） / Icon (menu)",
    )
    path: Mapped[str | None] = mapped_column(
        String(200),
        nullable=True,
        comment="前端路由（菜单专用） / Frontend route (menu)",
    )
    component: Mapped[str | None] = mapped_column(
        String(200),
        nullable=True,
        comment="前端组件（菜单专用） / Frontend component (menu)",
    )
    hidden: Mapped[bool] = mapped_column(
        Boolean,
        default=False,
        comment="是否隐藏菜单（仅做权限控制） / Hide in menu (ACL only)",
    )
