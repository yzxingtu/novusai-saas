...
class PluginLicense(BaseModel):
    ...
    __filterable__ = {"id": "id", "plugin_id": "plugin_id", "license_type": "license_type", "is_valid": "is_valid"}
    ...
    buyer_email: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="购买者邮箱",
    )
    issued_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="签发时间",
    )
    ...
    is_valid: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="是否有效",
    )
