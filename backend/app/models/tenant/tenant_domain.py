...
class TenantDomain(BaseModel):
    ...
    # 域名（全局唯一） / Hostname (globally unique)
    domain: Mapped[str] = mapped_column(
        String(255),
        unique=True,
        index=True,
        nullable=False,
        comment="域名 / Domain name",
    )
    ...
    verified_at: Mapped[datetime | None] = mapped_column(
        DateTime,
        nullable=True,
        comment="验证时间 / Verified at",
    )

    # 是否主域名（每个企业只能有一个主域名） / Primary domain flag
    is_primary: Mapped[bool] = mapped_column(
        Boolean,
        default=False,
        comment="是否主域名 / Primary domain",
    )
    ...
    ssl_status: Mapped[str] = mapped_column(
        ...
        comment="SSL 状态: none/pending/provisioning/active/failed/expired / SSL state",
    )

    # SSL 证书到期时间 / SSL expiry
    ssl_expires_at: Mapped[datetime | None] = mapped_column(
        DateTime,
        nullable=True,
        comment="SSL 证书到期时间 / SSL expires at",
    )

    # 验证记录值（用于 DNS TXT 验证） / DNS TXT verification token
    verification_token: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
        comment="域名验证 Token / Verification token",
    )
    ...
    __table_args__ = (
        # 复合索引：企业ID + 是否主域名 / Index on tenant + primary flag
