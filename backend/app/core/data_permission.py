...
class DataPermissionFilter:
    ...
    def apply(query: Select, model: type, current_user_id: int | None) -> Select:
        ...
        data_scope = ctx.get("scope_mode") or ctx.get("max_data_scope", DataScope.ALL.value)
        effective_scope_org_ids = (
            ctx.get("effective_scope_org_ids")
            or ctx.get("visible_org_ids")
            or ctx.get("all_visible_dept_ids")
            or []
        )
        custom_org_ids = ctx.get("custom_org_ids") or ctx.get("custom_dept_ids") or []
        ...
        if hasattr(model, "org_node_id"):
            org_field = model.org_node_id
        elif hasattr(model, "dept_id"):
            org_field = model.dept_id

        if data_scope in (DataScope.DEPT_ONLY.value, DataScope.DEPT_AND_CHILDREN.value):
            if org_field is None:
                return query.where(false())
            if not effective_scope_org_ids:
                return query.where(false())
            return query.where(org_field.in_(effective_scope_org_ids))

        if data_scope == DataScope.CUSTOM.value:
            if org_field is None:
                return query.where(false())
            target_ids = custom_org_ids or effective_scope_org_ids
            if not target_ids:
                return query.where(false())
            return query.where(org_field.in_(target_ids))
