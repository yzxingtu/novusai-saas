...
# Event handler type: receives BaseEvent subclass, returns None / 事件处理器类型：接收 BaseEvent 子类，返回 None
EventHandler = Callable[[BaseEvent], Coroutine[Any, Any, None]]


class _Subscription:
    """Subscription record containing handler and priority / 订阅记录，包含处理器和优先级"""
...
class EventBus:
    """
    Event Bus (in-process) / 事件总线（进程内）

    Features / 特性：
    - Async handlers (async def handler(event)) / 异步处理器
    - Priority ordering (lower number executes first) / 按优先级排序（数值越小越先执行）
    - Error isolation (single handler exception doesn't affect others) / 错误隔离（单个处理器异常不影响其他处理器）
    - Wildcard subscription (BaseEvent receives all events) / 支持通配符订阅（BaseEvent 接收所有事件）
    - Thread-safe singleton pattern / 线程安全的单例模式
    ...
    def get_instance(cls) -> "EventBus":
        """Get singleton instance (thread-safe) / 获取单例实例（线程安全）"""
    ...
    def subscribe(
        ...
        Args:
            event_type: Event type (BaseEvent subclass) / 事件类型（BaseEvent 子类）
            handler: Async handler function / 异步处理函数
            priority: Priority (lower number executes first, default 0) / 优先级（数值越小越先执行，默认 0）
        """
    ...
    async def publish(self, event: BaseEvent) -> None:
        """
        Publish event (executes all handlers serially).
        发布事件（串行执行所有处理器）。

        Error isolation: single handler exceptions are caught and logged without affecting others.
        错误隔离：单个处理器异常会被捕获并记录，不影响其他处理器。

        Args:
            event: Event instance / 事件实例
        """
        event_type = type(event)
        handlers_called = 0

        # Collect matching subscriptions: exact type + BaseEvent wildcard / 收集匹配的订阅：精确类型 + BaseEvent 通配
        ...
        # Re-sort by priority after merging / 合并后重新按优先级排序
    ...
    async def publish_nowait(self, event: BaseEvent) -> None:
        """
        Publish event (fire-and-forget, does not wait for handlers to complete).
        发布事件（fire-and-forget，不等待处理器完成）。

        Suitable for scenarios where handler results are not needed.
        Saves Task references to prevent GC from collecting during execution.
        适用于不关心处理结果的场景。
    ...
    def has_subscribers(self, event_type: type[BaseEvent]) -> bool:
        """Check if event type has subscribers / 检查事件类型是否有订阅者"""
        return len(self._subscribers.get(event_type, [])) > 0

    def subscriber_count(self, event_type: type[BaseEvent]) -> int:
        """Get subscriber count for event type / 获取事件类型的订阅者数量"""
        return len(self._subscribers.get(event_type, []))

    def clear(self) -> None:
        """Clear all subscriptions (for testing only) / 清除所有订阅（仅用于测试）"""
        self._subscribers.clear()


# Global convenience function / 全局便捷函数
def get_event_bus() -> EventBus:
    """Get global event bus instance / 获取全局事件总线实例"""
