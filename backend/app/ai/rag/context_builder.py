...
将检索到的分块拼接为 LLM 可用的上下文文本，
基于 Token 预算精确控制上下文长度
"""
...
# 默认 Token 预算参数
DEFAULT_CONTEXT_TOKEN_RATIO = 0.6  # RAG 上下文占剩余空间的比例
DEFAULT_OUTPUT_RESERVE = 500  # 至少预留给 LLM 生成的 token 数

# RAG 上下文拼接后缀（前缀在运行时通过 _() 获取）
RAG_CONTEXT_FOOTER = "\n---\n"
...
class RAGContext:
    ...
    rag_text: str  # 拼接后的上下文文本
    sources: list[SourceReference]  # 引用来源列表
    token_count: int  # 上下文消耗的 token 数
    chunk_count: int  # 实际纳入的分块数
...
class RAGContextBuilder:
    ...
    核心职责：
    1. 按 Token 预算从高分到低分逐块添加
    2. 生成带引用编号的上下文文本
    3. 输出 SourceReference 供 SSE 推送
    """
    ...
    def __init__(
        ...
        """
        Args:
            context_token_ratio: RAG 上下文占剩余空间的比例
            output_reserve: 预留给 LLM 生成的 token 数
        """
    ...
    def calculate_rag_budget(
        ...
        策略：
        1. 从总预算中扣除 system_prompt 和输出预留
        2. 剩余空间按 context_token_ratio 分配给 RAG
        3. 其余分配给对话历史

        Args:
            max_context_tokens: 模型总上下文窗口 token 数
            system_prompt_tokens: system_prompt 消耗的 token 数
            max_tokens: LLM max_tokens 参数

        Returns:
            (rag_budget, history_budget): RAG 和对话历史的 token 预算
        """
    ...
    def build_rag_context(
        ...
        Args:
            chunks: 检索结果列表（已按 score 降序排列）
            token_budget: 可用的 token 预算
