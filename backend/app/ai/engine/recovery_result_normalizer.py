    ...
    def _intent_cached_result(
        ...
        *,
        ...
        if intent_results and intent.intent_id in intent_results:
            ...
            if normalized:
                return normalized
        normalized = RecoveryResultNormalizer._normalize_cached_result(intent.cached_result)
        if normalized:
            return normalized
        ...
        for key in (
            ...
            normalized = RecoveryResultNormalizer._normalize_cached_result(metadata.get(key))
            if normalized:
                return normalized
        return None
