...
Relative paths like ``/api/public/attachments/{id}/access`` are not fetchable by
vendor APIs; we read bytes from storage (tenant-scoped) or HTTP fallback.
"""
...
_ATTACHMENT_ACCESS_RE = re.compile(
    r"/api/public/attachments/(\d+)/access",
    re.IGNORECASE,
)
...
def _attachment_id_from_url(raw: str) -> tuple[int | None, str | None]:
    ...
    path = parsed.path or ""
    m = _ATTACHMENT_ACCESS_RE.search(path)
    if not m:
        return None, None
    aid = int(m.group(1))
...
async def resolve_image_url_for_llm(
    ...
    *,
    ...
    aid, token = _attachment_id_from_url(url)
