...
class ConversationContextEngine(ContextEngine):
    ...
    async def assemble(
        ...
        logger.info(
            "[DEBUG] engine.py assemble: request.system_prompt_additions={}",
            request.system_prompt_additions,
        )
        if request.system_prompt_additions:
            system_prompt_additions.extend(request.system_prompt_additions)
            logger.info(
                "[DEBUG] engine.py: merged {} external additions, total now={}",
                len(request.system_prompt_additions),
                len(system_prompt_additions),
            )
