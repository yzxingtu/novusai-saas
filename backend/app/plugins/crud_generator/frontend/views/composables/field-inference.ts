...
export function getDefaultsByType(type: FieldType): Partial<FieldConfig> {
  switch (type) {
    case 'string': {
      return { max_length: 255, form_component: 'Input', search_op: 'ilike', search_component: 'Input' };
    }
    case 'text': {
      return { max_length: null, form_component: 'Textarea', search_op: 'ilike', search_component: 'Input', list_render: 'ellipsis' };
    }
    case 'integer': {
      return { form_component: 'InputNumber', search_op: 'eq', search_component: 'InputNumber' };
    }
    case 'float':
    case 'decimal': {
      return { form_component: 'InputNumber', search_op: 'eq', search_component: 'InputNumber', list_render: 'money' };
    }
    case 'boolean': {
      return { form_component: 'Switch', list_render: 'switch', search_op: 'eq', search_component: 'Select', nullable: false };
    }
    case 'datetime': {
      return { form_component: 'DatePicker', list_render: 'datetime', search_op: 'between', search_component: 'RangePicker', sortable: true };
    }
    case 'date': {
      return { form_component: 'DatePicker', list_render: 'date', search_op: 'between', search_component: 'RangePicker', sortable: true };
    }
    case 'json': {
      return { form_component: 'JsonEditor', search_op: 'ilike', search_component: 'Input', in_list: false };
    }
    case 'enum': {
      return { form_component: 'Select', list_render: 'tag', search_op: 'eq', search_component: 'Select' };
    }
    case 'file': {
      return { form_component: 'Upload', search_op: 'ilike', search_component: 'Input', in_list: false };
    }
    default: {
      return {};
    }
