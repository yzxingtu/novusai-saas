...
def _col_type_str(col_type: TypeEngine) -> str:
...
def get_project_graph_dict(*, force_refresh: bool = False) -> list[dict[str, object]]:
...
def get_graph_summary(*, force_refresh: bool = False) -> str:
    ...
    lines: list[str] = [f"## Project Models ({len(models)} tables)\n"]
    ...
    _render_group("BaseModel (Platform)", base_models)
    _render_group("TenantModel (Tenant)", tenant_models)
