...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", sa.Integer(), sa.ForeignKey("tenants.id", ondelete="CASCADE", name="fk_tenant_user_roles_tenant_id"), nullable=False),
        ...
        sa.Column("is_system", sa.Boolean(), nullable=False, server_default=sa.text("false"), comment="是否系统内置"),
    ...
    op.create_index("ix_tenant_user_roles_tenant_id", "tenant_user_roles", ["tenant_id"])

    # 2. 创建 tenant_user_role_permissions 关联表
    op.create_table(
        "tenant_user_role_permissions",
        sa.Column("role_id", sa.Integer(), sa.ForeignKey("tenant_user_roles.id", ondelete="CASCADE", name="fk_turp_role_id"), primary_key=True),
        sa.Column("permission_id", sa.Integer(), sa.ForeignKey("permissions.id", ondelete="CASCADE", name="fk_turp_permission_id"), primary_key=True),
    )
    ...
    op.add_column(
        ...
        sa.Column(
            "role_id",
            sa.Integer(),
            sa.ForeignKey("tenant_user_roles.id", ondelete="SET NULL", name="fk_tenant_users_role_id"),
            nullable=True,
            comment="用户角色 ID",
        ),
    )
