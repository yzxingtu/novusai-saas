"""中文: 重新收敛已升级环境的内置周期任务契约。

EN: Re-canonicalize built-in periodic task contracts for upgraded environments.

Revision ID: 20260508_0036_task_contract
Revises: 20260508_0035_ai_proto_cfg
...
revision: str = "20260508_0036_task_contract"
...
def _canonicalize_notification_cleanup_kwargs(bind: sa.Connection) -> None:
    ...
    rows = (
        bind.execute(
            sa.select(task_definitions.c.id).where(
                sa.or_(
    ...
    for row in rows:
        bind.execute(
            sa.update(task_definitions)
            .where(task_definitions.c.id == int(row["id"]))
            .values(default_kwargs={}, **_now_value(columns))
        )
    ...
    for row in binding_rows:
        ...
        bind.execute(
            sa.update(task_bindings)
            .where(task_bindings.c.id == int(row["id"]))
            .values(kwargs_override={}, **_now_value(binding_columns))
        )
