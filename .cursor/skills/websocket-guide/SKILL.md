...
| `/tenant` | 企业管理员 | `tenant_admin` | `/tenant/*` |
| `/user` | 企业业务用户 | `tenant_user` | `/api/user/*` |
...
| `tenant:{tenant_id}` | 企业广播 | on_connect（tenant/user namespace） |
...
GET /admin/ws/presence/tenant/{id}        → 指定企业管理员在线列表
GET /tenant/ws/presence                   → 当前企业管理员在线列表
```
