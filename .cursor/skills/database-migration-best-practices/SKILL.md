...
**统一格式：** 新迁移必须使用 `YYYYMMDD_HHMM_short_description.py`，禁止混用日期+哈希（如 `20260212_ef3543e8d16b_xxx.py`）或纯描述无序号。
...
### 9. Raw SQL 参数化（禁止 f-string 拼接）

迁移中若需执行动态 SQL，**禁止**使用 `text(f"...{var}...")` 拼接，必须用 `text("...").bindparams(param=value)` 参数化，防止 SQL 注入与转义问题。

```python
from sqlalchemy import text

# ❌ 错误 — f-string 拼接存在 SQL 注入风险
...
- [ ] 使用 `datetime.now(timezone.utc)` 替代已废弃的 `datetime.utcnow`（Python 3.12+）
- [ ] **禁止 `text(f"...")` 拼接 SQL**：raw SQL 必须用 `text("...").bindparams(param=value)` 参数化，防 SQL 注入
...
### 15. Codegen 自动迁移与手写迁移共存

Codegen 使用 `--auto-migrate`（默认开启）自动执行 `alembic revision --autogenerate`。它与手写迁移共存的规则：

- **Codegen 迁移** — 由 `novusai codegen generate --auto-migrate` 自动生成，文件内含 `codegen_source` / `codegen_resource` 元数据
- **手写迁移** — 由 `novusai db autogenerate -m "..."` 或 `novusai db revision -m "..."` 手动创建

**共存注意事项：**

1. **不要同时操作** — 如果正在 codegen，不要同时手动 autogenerate
2. **Codegen 生成前会 `upgrade head`** — 确保 DB 处于最新，不会与未执行的手写迁移冲突
3. **Codegen 生成前会 `purge_orphaned_alembic_stamps`** — 清理插件卸载等残留
4. **多人协作** — 两人同时 codegen 或一人 codegen 一人手写，可能产生多 head，用 `novusai db merge -m "merge"` 合并
...
6. **Codegen 与手写迁移不要同时操作**，避免多 head
