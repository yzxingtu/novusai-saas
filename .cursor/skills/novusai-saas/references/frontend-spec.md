...
const data = await requestClient.post('/admin/users', userData, {
  ...
  // 如需自定义提示文案，必须使用 $t()，不要硬编码中文
  // If you need custom success text, use $t() instead of hard-coded Chinese
...
const result = await smartUploadFile(file, {
  visibility: 'private',
  onProgress: (progress) => {
    // 更新 UI 状态 / Update upload UI state
...
await requestClient.postSSE(
  '/tenant/ai/agent-chat/1/chat/stream',
...
formatTimeOnly('2026-01-15T10:30:00');   // '10:30:00'
formatRelativeTime('2026-01-15T10:30:00'); // '3 天前'
generateCode({ length: 8 });             // 'A3Km9Xp7'
```

### 时间显示规范

**禁止**使用原生 `toLocaleString` / `toLocaleDateString` / `toLocaleTimeString`，必须使用上述工具函数。

| 场景 | 函数 | 示例 |
|------|------|------|
| 列表页时间列 | `formatRelativeTime` + `formatDate` Tooltip | `<Tooltip :title="formatDate(row.createdAt)"><span>{{ formatRelativeTime(row.createdAt) }}</span></Tooltip>` |
| 详情页/抽屉 | `formatDate` | `{{ formatDate(detail.createdAt) }}` |
| 仅日期（过期日期等） | `formatDateOnly` | `{{ formatDateOnly(row.expiresAt) }}` |
| 消息时间戳 | `formatTimeOnly` | `{{ formatTimeOnly(msg.created_at) }}` |
...
export function useGridFormSchema() {
  return [
    searchInput('code', '企业编码'), // => filter[code][ilike]
...
export function useFormSchema(isEdit: boolean) {
  return [
    inputField('name', '企业名称', { required: true, maxLength: 100 }),
...
| 元素 | 适用场景 | 示例 |
| --- | --- | --- |
| **首字头像** | 名称、用户、企业等需要辨识度的字段 | 圆形/方形，背景色根据内容生成 |
