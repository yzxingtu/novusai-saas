...
**父资源 Controller 必须先于子资源导入**。例如 `admin/__init__.py` 中，`tenants` 必须先于 `tenant_admins`、`tenant_domains` 导入。否则子资源的 `parent_resource="tenant"` 在注册时找不到 `menu:admin.tenant`，导致权限 `parent_id=null` 成为孤立节点。
...
> 新增用户端页面时，不要继续扩散该命名；前端当前规范入口以 tenant 域名下的 `/`、`/agents`、`/ai-chat`、`/help`、`/settings/*` 为准，`/home` 仅保留为兼容 alias，平台域名下的 `/` 则是公共网关。
