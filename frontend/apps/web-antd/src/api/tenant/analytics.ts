...
export async function getTenantModelDistributionApi(
  params?: DateRangeParams,
...
export async function getTenantAgentRankingApi(
  topN = 10,
  params?: DateRangeParams,
): Promise<AgentRankingItem[]> {
  return requestClient.get<AgentRankingItem[]>(`${API_PREFIX}/agent-ranking`, {
    params: { top_n: topN, ...params },
  });
}

export async function getTenantCostTrendApi(
  params?: DateRangeParams,
): Promise<CostTrendItem[]> {
  return requestClient.get<CostTrendItem[]>(`${API_PREFIX}/cost-trend`, {
    params,
  });
}
