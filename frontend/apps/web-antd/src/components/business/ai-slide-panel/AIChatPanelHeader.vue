...
<template>
  <div
    class="ai-panel-header flex shrink-0 items-center justify-between gap-3 border-b border-border/24 px-2.5 py-2"
  >
    <div class="flex min-w-0 flex-1 items-center gap-2.5">
      <div
        class="ai-panel-header-orb flex size-7 shrink-0 items-center justify-center rounded-[16px] text-primary"
      >
      ...
      <div class="min-w-0 flex-1">
        <div class="flex min-w-0 items-center gap-1.5 leading-none">
          <span
            class="truncate text-[11.5px] font-semibold tracking-[0.01em] text-foreground/88"
          >
            {{ panelTitle }}
          </span>
          <span
            v-if="routing"
            class="routing-badge relative inline-flex items-center gap-1 overflow-hidden rounded-full px-1.5 py-0.5 text-[8.5px] font-medium text-primary"
          >
        ...
        <div class="mt-1 flex min-w-0 items-center gap-1.5">
          <div
            ...
            class="ai-panel-route-pill inline-flex min-w-0 max-w-full items-center gap-1.5 rounded-full px-2 py-1 text-[9.5px] text-muted-foreground/74"
          >
            <span
              class="flex size-4 shrink-0 items-center justify-center rounded-full bg-primary/12 text-primary"
            >
          ...
          <div
            ...
            class="truncate text-[9.5px] text-muted-foreground/58"
          >
            {{ headerConversationSummary }}
          </div>
        </div>
    ...
    >
      <div
        class="ai-panel-header-actions flex shrink-0 items-center gap-0.5 rounded-[14px] px-0.5 py-0.5"
      >
        <Tooltip
          :title="
        ...
        >
          <button
            type="button"
            class="flex size-6 items-center justify-center rounded-[10px] transition-colors hover:bg-muted/70"
            :class="
              docked
        ...
        >
          <button
            type="button"
            class="flex size-6 items-center justify-center rounded-[10px] text-muted-foreground/72 transition-colors hover:bg-muted/70 hover:text-foreground/84"
            :aria-label="
              mode === 'full'
                ? $t('common.aiPanel.exitFullscreen')
                : $t('common.aiPanel.fullscreen')
        ...
        <Tooltip :title="$t('common.aiPanel.minimize')">
          <button
            ...
            @click="emit('minimize')"
          >
            <IconifyIcon icon="lucide:minus" class="size-3" />
          </button>
        </Tooltip>
        <Tooltip :title="$t('common.aiPanel.close')">
          <button
            type="button"
            class="flex size-6 items-center justify-center rounded-[10px] text-muted-foreground/72 transition-colors hover:bg-muted/70 hover:text-destructive"
...
.ai-panel-header-orb {
  ...
  box-shadow:
    0 14px 24px -24px hsl(var(--primary) / 0.38),
