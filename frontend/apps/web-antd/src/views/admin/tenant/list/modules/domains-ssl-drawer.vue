...
const sslDetail = ref<null | SslDetailResponse>(null);
      ...
      <template v-if="sslDetail">
        ...
        <div class="mb-4">
          ...
          <Descriptions :column="1" bordered size="small">
            ...
            <Descriptions.Item
              v-if="sslDetail.validFrom"
              :label="$t('admin.tenant.domain.ssl.info.validFrom')"
            >
              {{ sslDetail.validFrom }}
            </Descriptions.Item>
            <Descriptions.Item
              v-if="sslDetail.validTo"
            ...
            >
              ...
              <span
                v-if="sslDetail.nextRenewAt"
                class="ml-2 text-xs text-gray-400"
              >
                {{ $t('admin.tenant.domain.ssl.autoRenew.nextRenew') }}:
                {{ sslDetail.nextRenewAt }}
              </span>
            </Descriptions.Item>
          </Descriptions>
        </div>
        ...
        <template v-if="isPlatformMode && hasActiveCert">
          <div class="mb-4">
            <h4 class="mb-3 text-sm font-medium">
              {{ $t('admin.tenant.domain.ssl.certificate.label') }}
            </h4>
            <div class="flex flex-col gap-3">
              ...
              <div class="rounded border border-gray-200 p-3">
                ...
                <code
                  class="block max-h-24 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-2 text-xs"
                >
              ...
              <!-- 证书链 -->
              <div
                v-if="sslDetail.certificateChain"
        ...
        <template v-if="isCustomMode && hasActiveCert">
          <div class="mb-4">
            <h4 class="mb-3 text-sm font-medium">
              {{ $t('admin.tenant.domain.ssl.certManage') }}
            </h4>
        ...
        <div>
          <h4 class="mb-3 text-sm font-medium">
            {{ $t('admin.tenant.domain.ssl.sourceSelect') }}
          </h4>
                ...
                <Radio value="platform" class="w-full">
                  <div class="ml-2">
                    <div class="flex items-center gap-2">
                      ...
                      <Tag v-if="sslDetail.type === 'platform'" color="blue">
                        {{ $t('shared.common.current') }}
                      </Tag>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">
                      {{ $t('admin.tenant.domain.ssl.platformHint') }}
                    </p>
                    <div
                      v-if="selectedType === 'platform'"
                ...
                <Radio value="custom" class="w-full">
                  <div class="ml-2">
                    <div class="flex items-center gap-2">
                      ...
                      <Tag v-if="sslDetail.type === 'custom'" color="blue">
                        {{ $t('shared.common.current') }}
                      </Tag>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">
                      {{ $t('admin.tenant.domain.ssl.uploadHint') }}
                    </p>
                    <div v-if="selectedType === 'custom'" class="mt-3">
                      <Button size="small" @click.stop="onUploadCert">
                        ...
                        {{
                          sslDetail.type === 'custom'
                            ? $t('admin.tenant.domain.ssl.actions.reupload')
