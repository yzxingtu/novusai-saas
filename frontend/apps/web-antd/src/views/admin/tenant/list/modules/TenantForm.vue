 ...
 * - Edit mode: backend camelCase -> form snake_case
 * - 编辑模式：后端 camelCase -> 表单 snake_case
 * - Submit: form snake_case -> API snake_case (empty values -> null)
...
// Extra admin fields for create mode / 新建时额外的管理员字段
const adminFields = ['admin_username', 'admin_email', 'admin_password'];

// All fields / 所有字段
const allFields = [...baseFields, ...adminFields];

// Form (plan dropdown auto-loaded by ApiSelect) / 表单（套餐下拉由 ApiSelect 自动加载）
...
const { Drawer, isEdit } = useCrudDrawer<TenantInfo>({
  ...
  // Use all fields (create schema includes admin fields, edit does not) / 使用所有字段（新建时 schema 会包含管理员字段，编辑时不包含）
  fields: allFields,
  // Custom transform: exclude admin fields in edit mode / 自定义 transform：编辑时排除管理员字段
