...
function getEnValue(key: string): string {
  ...
  if (parts[0] === 'createTitle') return $t(`${T}.translate.createTitle`, { name: props.config.display_name_en || '' });
  if (parts[0] === 'editTitle') return $t(`${T}.translate.editTitle`, { name: props.config.display_name_en || '' });
  ...
  if (parts[0] === 'message') {
    ...
    const keyMap: Record<string, string> = {
      createSuccess: `${T}.translate.createSuccess`,
      updateSuccess: `${T}.translate.updateSuccess`,
      deleteSuccess: `${T}.translate.deleteSuccess`,
      deleteConfirm: `${T}.translate.deleteConfirm`,
    };
    return keyMap[msgKey] ? $t(keyMap[msgKey]!) : '';
  }
