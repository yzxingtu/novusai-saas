...
const { loadGraph, models, getModelOptions } = useProjectGraph();
onMounted(() => loadGraph());

const modelAutoCompleteOptions = computed(() =>
  getModelOptions().map((o) => ({ value: o.label.split(' (')[0] ?? o.label })),
);
...
/** target_model → target_table 自动联动 (uses project graph if available) */
function onTargetModelChange(index: number, val: string) {
  const patch: Partial<RelationConfig> = { target_model: val };
  if (val) {
    const graphModel = Object.values(models.value).find(
      (m) => m.class_name === val,
    );
    if (graphModel) {
      patch.target_table = graphModel.table_name;
    } else {
      const snake = val
        .replace(/([A-Z])/g, '_$1')
        .toLowerCase()
      ...
      <template #bodyCell="{ column, index }">
        ...
        <template v-else-if="column.dataIndex === 'target_model'">
          <AutoComplete
            ...
            size="small"
            style="width: 100%"
            :filter-option="(input: string, option: { value: string }) => option.value.toLowerCase().includes(input.toLowerCase())"
            @change="(v: unknown) => onTargetModelChange(index, String(v ?? ''))"
          />
        </template>
