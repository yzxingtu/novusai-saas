...
export const PRESET_REGISTRY: Record<ListRenderPreset, PresetDefinition> = {
  ...
  money: {
    ...
    props: {},
    previewTemplate:
      '<span class="font-mono tabular-nums">{{ $t("admin.dev.crudGenerator.currencySymbol") }} {{ Number(record.{field}).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>',
    codegenTemplate:
      '<span class="font-mono tabular-nums">{{ $t("common.currencySymbol") }} {{ formatMoney(record.{field}) }}</span>',
    imports: [],
  },
