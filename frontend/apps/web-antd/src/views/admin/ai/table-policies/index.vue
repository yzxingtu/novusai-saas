...
const { Grid, FormDrawer, onRefresh } = useCrudPage<AITablePolicyInfo>({
  ...
  search: {
    defaultOpen: false,
    quickSearch: {
      defaultField: 'filter[table_name][ilike]',
      fields: ['filter[table_name][ilike]', 'filter[label][ilike]'],
    },
  },
  formComponent: Form,
...
<template>
  <Page auto-content-height content-class="flex flex-col gap-4 !p-4">
    <AIGatewayQuickStartHero
      :current-title="$t('admin.ai.tablePolicy.title')"
    />
