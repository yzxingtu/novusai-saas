...
function mapSqlType(raw: string): FieldType {
  ...
  return (base in SQL_TYPE_MAP ? SQL_TYPE_MAP[base] : undefined) ?? 'string';
}
...
const DDL_COLUMN_RE =
  /^\s*"?(\w+)"?\s+([\w\s()]+?)(?:\s+(NOT\s+NULL|NULL))?(?:\s+DEFAULT\s+(.+?))?(?:\s+(PRIMARY\s+KEY|UNIQUE))?(?:\s*,?\s*)$/i;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!match) {
      ...
      if (simpleMatch) {
        ...
        errors.push({ line: lineNum, field: '', reason: `Cannot parse column definition` });
      }
      continue;
    }
    ...
    fields.push({
      ...
      max_length: extractMaxLength(typeStr),
      default_value: defaultVal ? defaultVal.replace(/'/g, '').trim() || null : null,
      index: isPK,
      primary_key: isPK,
    });
  }
...
export function parseJSON(input: string): ParseResult {
  ...
  try {
    ...
    errors.push({ line: 1, field: '', reason: 'Invalid JSON' });
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (!item || typeof item !== 'object') {
      errors.push({ line: i + 1, field: '', reason: 'Item is not an object' });
      continue;
    }

    if (!item.name || typeof item.name !== 'string') {
      errors.push({ line: i + 1, field: '', reason: 'Missing or invalid "name"' });
      continue;
    }
    ...
    const mappedType: FieldType = Object.values(SQL_TYPE_MAP).includes(typeStr as FieldType)
      ? (typeStr as FieldType)
      : mapSqlType(typeStr);
...
export function parseCSV(input: string): ParseResult {
  ...
  if (nameIdx === -1) {
    errors.push({ line: 1, field: '', reason: 'CSV header must contain "name" column' });
    return { fields, errors, source: 'csv' };
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!name) {
      errors.push({ line: i + 1, field: '', reason: 'Empty name' });
      continue;
    }
    ...
    const toBool = (val: string | undefined): boolean =>
      val !== undefined && ['true', '1', 'yes', 'y'].includes(val.toLowerCase());
...
export function parsedFieldToConfig(pf: ParsedField): FieldConfig {
  return {
    ...
    list_sortable: false,
    in_form: true,
    form_component: pf.type === 'text' ? 'Textarea'
      : pf.type === 'boolean' ? 'Switch'
      : pf.type === 'integer' || pf.type === 'float' || pf.type === 'decimal' ? 'InputNumber'
      : pf.type === 'datetime' || pf.type === 'date' ? 'DatePicker'
      : pf.type === 'json' ? 'JsonEditor'
      : pf.type === 'enum' ? 'Select'
      : 'Input',
    form_group: null,
...
export function mergeFields(
  ...
  for (const field of incoming) {
    const existField = existingMap.get(field.name);

    if (!existField) {
      result.push(field);
      added.push(field.name);
    } else if (mode === 'overwrite_same') {
      const idx = result.findIndex((f) => f.name === field.name);
      if (idx >= 0) {
        result[idx] = field;
