...
async function fetchPreview() {
  ...
  try {
    const res = await previewCrudGenerateApi(
      props.config as unknown as Record<string, unknown>,
    );
    const payload = (res as unknown as { data: { files: PreviewFileItem[]; ddl_preview: string; warnings: string[] } }).data;
    previewFiles.value = payload.files || [];
    ddlContent.value = payload.ddl_preview || '';
