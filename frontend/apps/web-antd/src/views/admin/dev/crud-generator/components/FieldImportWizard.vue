...
// ---- Wizard state ----
const step = ref(0);
type ImportSource = 'ddl' | 'json' | 'csv';
        ...
        <template #description>
          <ul class="list-inside list-disc text-xs">
            <li v-for="(err, idx) in parseResult.errors" :key="idx">
              {{ $t(`${T}.errorLine`, { line: err.line }) }}: {{ err.reason }}
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.dataIndex === 'selected'">
            <input
              :checked="selectedFields.has((record as ParsedField).name)"
              :disabled="(record as ParsedField).primary_key"
              type="checkbox"
              @change="toggleField((record as ParsedField).name)"
            />
          </template>

          <template v-else-if="column.dataIndex === 'name'">
            <span class="font-mono text-xs">{{ (record as ParsedField).name }}</span>
          </template>
          ...
          <template v-else-if="column.dataIndex === 'required'">
            <span v-if="(record as ParsedField).required" class="icon-[lucide--check] size-4 text-success" />
          </template>

          <template v-else-if="column.dataIndex === 'nullable'">
            <span v-if="(record as ParsedField).nullable" class="icon-[lucide--check] size-4 text-muted-foreground" />
          </template>

          <template v-else-if="column.dataIndex === 'status'">
            <Tag
              :color="getFieldStatus(record as ParsedField) === 'new' ? 'green' : getFieldStatus(record as ParsedField) === 'exists' ? 'orange' : 'default'"
              size="small"
            >
              {{ $t(`${T}.status_${getFieldStatus(record as ParsedField)}`) }}
            </Tag>
          </template>
        </template>
      </Table>
