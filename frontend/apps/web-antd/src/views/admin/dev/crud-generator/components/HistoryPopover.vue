...
async function fetchRecent() {
  ...
  try {
    ...
    const data = (res as unknown as { data: { items: CrudRecordInfo[] } }).data;
...
async function handleRestore(record: CrudRecordInfo) {
  try {
    ...
    const res = await getCrudRecordConfigApi(record.id);
    const configData = (res as unknown as { data: CrudConfig }).data;
    if (configData) {
      emit('restoreConfig', configData);
      open.value = false;
    }
  } catch {
    // ignore
