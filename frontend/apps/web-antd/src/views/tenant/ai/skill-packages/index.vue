...
async function handleCustomUpload(options: UploadRequestOption) {
  ...
  try {
    ...
    message.success($t('tenant.ai.skillPackage.messages.uploadSuccess'));
...
function onCreatePackage() {
  packageFormApi
    .setData({
      mode: 'add',
      _resource: '/tenant/ai/skill-packages',
      _defaults: { is_active: true, sort_order: 0 },
    })
    .open();
}

function onEditPackage(pkg: TenantSkillPackageInfo) {
  packageFormApi
    .setData({ ...pkg, mode: 'edit', _resource: '/tenant/ai/skill-packages' })
    .open();
}

async function onDeletePackage(pkg: TenantSkillPackageInfo) {
  Modal.confirm({
    title: $t('tenant.common.confirmDelete'),
    onOk: async () => {
      try {
...
function onEditSkill(row: SkillInfo) {
  skillFormApi
    .setData({ ...row, mode: 'edit', _resource: '/tenant/ai/skills' })
    .open();
}
...
// ==================== 技能操作 ====================
async function onDeleteSkill(row: SkillInfo) {
  Modal.confirm({
    title: $t('tenant.common.confirmDelete'),
    onOk: async () => {
      try {
...
<template>
  ...
  >
    ...
    <Modal
      ...
      :title="$t('tenant.ai.skillPackage.uploadZip')"
      :footer="null"
    ...
    >
      ...
      <template #extra>
        <Space :size="4">
          ...
          <Tooltip :title="$t('tenant.ai.skillPackage.create')">
            <Button
              v-access:code="['skill_package:create']"
              type="text"
              size="small"
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'action'">
              <Space>
                <Tooltip :title="$t('tenant.ai.skill.testBtn')">
                  <Button
                    type="link"
                    size="small"
                    @click="onTestSkill(record as SkillInfo)"
                  >
                ...
                <Button
                  ...
                  type="link"
                  size="small"
                  @click="onEditSkill(record as SkillInfo)"
                >
                  {{ $t('common.edit') }}
                </Button>
                <Button
                  ...
                  size="small"
                  danger
                  @click="onDeleteSkill(record as SkillInfo)"
                >
                  {{ $t('tenant.common.delete') }}
                </Button>
              </Space>
            </template>
          </template>
