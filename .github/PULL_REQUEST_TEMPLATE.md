...
- [ ] This change is customer-only and is intentionally limited to sanctioned downstream paths: `business/<project-code>/`, `customer/<project-code>/`, or `extensions/<package-code>/`.
...
Customer or vertical business modules belong under `business/<project-code>/`.
Deployment, config, seed, branding, and delivery overlays belong under
`customer/<project-code>/`. Reusable non-core plugins, connectors, and skill
packages belong under `extensions/<package-code>/`.
...
- [ ] Customer business code is isolated under `business/<project-code>/` and is not mixed into Yudi core directories.
