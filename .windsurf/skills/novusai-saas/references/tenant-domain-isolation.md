...
**原理**：访问 `as.dakkii.cn:5666` 时，API 请求的 `Host: as.dakkii.cn:8000`，后端 TenantMiddleware 通过 Host header 解析出租户。
...
async detectDomainType(): Promise<void> {
  ...
  try {
    ...
    this.isDomainTenantDomain = true;                       // 200 → 租户域名
...
|------------------------|------|
| `true` | 租户域名（API 返回 200，找到租户） |
| `false` | 平台域名（API 返回 404，无租户） |
...
| 生产（`DEBUG=False`） | 必须在 `CORS_ORIGINS` 中加入所有租户域名，否则跨域请求 403 |

---

## 七、安全检查清单

- [ ] 后端租户认证端点正确提取 `tenant_id_from_ctx`（不依赖 URL 参数）
