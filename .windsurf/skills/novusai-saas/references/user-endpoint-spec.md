...
```
┌─────────────────────────────────────────────────────────────┐
│  Logo + Brand  │  Dashboard  AI Chat  Analytics  │  🔔  👤  │  ← 56px 导航栏
├─────────────────────────────────────────────────────────────┤
│                                                             │
...
- 56px 水平导航栏：Logo + Brand（租户配置）+ 导航链接（RBAC 动态菜单）+ 通知铃铛 + 用户头像下拉
...
```
用户访问 https://abc.app.novusai.com/login
    │
    ├─ 1. Router Guard 检测 currentEndpoint = 'user'
    ├─ 2. 调用 loadTenantConfig() → GET /api/public/tenant/config
    │     └─ 后端 TenantMiddleware 从 Host 提取 tenant_code
    │     └─ 返回品牌/验证码/登录方式/安全策略
    ├─ 3. 前端应用品牌配置（logo/主题色/站点名）
...
USER_DIRECTORY_MENUS: list[PermissionMeta] = [
    PermissionMeta(
        ...
        scope=PermissionScope.USER,        # ← 新增 scope
        resource="menu",
...
export function userLoginApi(data) {
  return requestClient.post('/user/auth/login/json', data);
}
export function userRegisterApi(data) {
  return requestClient.post('/user/auth/register', data);
}
export function getUserProfileApi() {
  return requestClient.get('/user/auth/me');
}
export function updateUserProfileApi(data) {
  return requestClient.put('/user/auth/profile', data);
}
export function forgotPasswordApi(data) {
  return requestClient.post('/user/auth/forgot-password', data);
}
export function resetPasswordApi(data) {
  return requestClient.post('/user/auth/reset-password', data);
}
```
...
Key 前缀：`user.common.*` / `user.dashboard.*` / `user.profile.*` / `user.auth.*`
